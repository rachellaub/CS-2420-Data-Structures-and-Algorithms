...
public:
	...
	HashTable() { arr = new list< KeyValuePair<T>>[HashTable<T>::NUM_BUCKETS]; }
	...
	void create(const string& key, const T& item) {
		...
		arr[hash(key)].push_back(KeyValuePair<T>(key, move(item)));
	}

	void create(const string& key, T&& item) {

		arr[hash(key)].emplace_back(key, std::move(item));
	}

	T retrieve(const string& key) {
		for (auto it = arr[hash(key)].begin(); it != arr[hash(key)].end(); ++it) {
			if (it->key == key) {
				return it->value;
			}
		}
...
private:
	...
	//TODO: Make an array of linked lists of KeyValuePair objects.  
