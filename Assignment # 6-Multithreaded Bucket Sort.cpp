...
void BucketsCollection::copyOneBucketsIntoAnotherBuckets(BucketsCollection& smallBucket) {
	...
	// this --- the global buckets
	//smallBucket -- the local buckets
	myMutex.lock();
	for (int i = 0; i < numBuckets; i++)
	{
...
void placeIntoBuckets(int threadID) {
	...
	unsigned int range = listSize / numThreads;
	unsigned int startIndex = range * threadID;
	unsigned int endIndex = range * (threadID + 1);
...
void combineBuckets(int threadID) {
	...
	unsigned int size = globalBuckets->getNumItemsInABucket(threadID);

	for (int i = 0; i < threadID; i++) {
		index = index + globalBuckets->getNumItemsInABucket(i);
	}

	for (int j = 0; j < size; j++) {
		list[index] = tempArr[j];
		index++;
	}
...
void bucketSort(bool displayOutput) {
	...
	for (int i = 0; i < numThreads; i++) {
		threads[i] = thread(placeIntoBuckets, i); //this launches a child thread
	}
	for (int i = 0; i < numThreads; i++) {
		threads[i].join();				//a barrier to wait for this thread 
	}
	...
	for (int i = 0; i < numThreads; i++) {
		threads[i] = thread(sortEachBucket, i); //this launches a child thread
	}
	for (int i = 0; i < numThreads; i++) {
		threads[i].join();				//a barrier to wait for this thread 
	}
	...
	for (int i = 0; i < numThreads; i++) {
		threads[i] = thread(combineBuckets, i); //this launches a child thread
	}
	for (int i = 0; i < numThreads; i++) {
		threads[i].join();				//a barrier to wait for this thread 
	}
